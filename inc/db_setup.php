    ...
    try {
        ...
        $pdo->exec("
            CREATE TABLE IF NOT EXISTS firms (
                ...
                created_at TEXT    NOT NULL DEFAULT (datetime('now', 'localtime')) -- localtime eklendi
            );
        ");
        ...
        $pdo->exec("
            CREATE TABLE IF NOT EXISTS trips (
                ...
                depart_at   TEXT    NOT NULL, -- Format: 'YYYY-MM-DD HH:MM'
                arrive_at   TEXT,             -- Format: 'YYYY-MM-DD HH:MM'
        ...
        $pdo->exec("
            CREATE TABLE IF NOT EXISTS coupons (
                id               INTEGER PRIMARY KEY AUTOINCREMENT,
                firm_id          INTEGER, -- NULL ise global (tüm firmalar)
                code             TEXT    NOT NULL,
                ...
                created_at       TEXT    NOT NULL DEFAULT (datetime('now', 'localtime')), -- localtime eklendi
                FOREIGN KEY(firm_id) REFERENCES firms(id) ON DELETE CASCADE,
                -- Bir firma için aynı kod tekrar edemez, global kuponlar (firm_id IS NULL) için de kod unique olmalı
                UNIQUE (code, firm_id),
                -- En az bir indirim türü belirtilmeli
        ...
        $pdo->exec("
            CREATE TABLE IF NOT EXISTS tickets (
              ...
              seat_no       INTEGER NOT NULL, -- Koltuk numarası (iptalde negatif olabilir)
              price_cents   INTEGER NOT NULL CHECK (price_cents >= 0),
              coupon_id     INTEGER,
              status        TEXT    NOT NULL CHECK (status IN ('purchased', 'cancelled')),
              purchased_at  TEXT    DEFAULT (datetime('now', 'localtime')), -- localtime eklendi
              cancelled_at  TEXT,             -- İptal tarihi
        ...
        $pdo->exec("
            CREATE TABLE IF NOT EXISTS wallet_tx (
                ...
                amount_cents  INTEGER NOT NULL, -- Pozitif: yükleme/iade, Negatif: harcama (veya tam tersi, koda göre ayarla!)
                ref_ticket_id INTEGER,          -- İlişkili bilet ID'si (varsa)
                note          TEXT,             -- İşlem açıklaması
                created_at    TEXT    NOT NULL DEFAULT (datetime('now', 'localtime')), -- localtime eklendi
                FOREIGN KEY (user_id)       REFERENCES users(id)   ON DELETE CASCADE,
        ...
        if (!$checkStmt->fetchColumn()) {
            error_log("Varsayılan admin kullanıcısı bulunamadı, oluşturuluyor (E-posta: $adminEmail, Şifre: password)...");
            ...
            error_log("Varsayılan admin kullanıcısı (E-posta: $adminEmail) zaten mevcut.");
        }

    
        $pdo->commit();
        error_log("Veritabanı kurulum transaction'ı başarıyla commit edildi.");

    } catch (\PDOException $e) {
     
        $pdo->rollBack();
        error_log("Veritabanı kurulumu BAŞARISIZ! Transaction geri alındı. Hata: " . $e->getMessage());
